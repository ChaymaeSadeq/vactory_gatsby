...
import {ParagraphsMultipleAccordion, ParagraphsMultipleTab, ParagraphsTemplate} from "vactory-gatsby-ui"
...
export const ParagraphsMultiple = ({type = 'tab', tabs = [], hasAMP, ...rest}) => {
    ...
    const components = tabs.map(p => {
        ...
        const widgets = field_tab_templates.map(widget => {
            ...
            return <div key={widget.id} data-df-id={widget_id}>
                <ParagraphsTemplate
                    id={widget_id}
                    hasAMP={hasAMP}
